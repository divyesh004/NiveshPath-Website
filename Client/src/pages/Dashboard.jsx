    ...
    <div className="page-container">
      ...
      <main className="content-container pb-24 sm:pb-12"> {/* Increased padding bottom for mobile nav */}
        ...
        <div className="mb-6 sm:mb-8">
          <h2 className="section-title text-2xl sm:text-3xl">Welcome, {userName}!</h2>
          <p className="section-description text-responsive">Here's an overview of your financial journey.</p>
        </div>
        
        {/* Quick Actions */}
        <div className="mb-6 sm:mb-8">
          <h3 className="text-lg font-medium text-primary dark:text-white mb-3 sm:mb-4">Quick Actions</h3>
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <Link to="/chatbot" className="card p-4 flex items-center space-x-3 hover:shadow-md transition-shadow">
              ...
              <div>
                <h4 className="text-primary dark:text-white font-medium">Ask FinBot</h4>
                <p className="text-sm text-gray-600 dark:text-gray-400">Get financial advice</p>
              </div>
            </Link>
            
            <Link to="/tools/sip-calculator" className="card p-4 flex items-center space-x-3 hover:shadow-md transition-shadow">
              ...
              <div>
                <h4 className="text-primary dark:text-white font-medium">SIP Calculator</h4>
                <p className="text-sm text-gray-600 dark:text-gray-400">Plan your investments</p>
              </div>
            </Link>
            
            <Link to="/tools/emi-calculator" className="card p-4 flex items-center space-x-3 hover:shadow-md transition-shadow">
              <div className="flex-shrink-0 h-10 w-10 rounded-full bg-secondary flex items-center justify-center text-white">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div>
                <h4 className="text-primary dark:text-white font-medium">EMI Calculator</h4>
                <p className="text-sm text-gray-600 dark:text-gray-400">Calculate loan EMIs</p>
              </div>
            </Link>
            
            <Link to="/tools/budget-planner" className="card p-4 flex items-center space-x-3 hover:shadow-md transition-shadow">
              <div className="flex-shrink-0 h-10 w-10 rounded-full bg-secondary flex items-center justify-center text-white">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div>
                <h4 className="text-primary dark:text-white font-medium">Budget Planner</h4>
        ...
        {/* Market Data */}
        <div className="mb-6 sm:mb-8">
          <h3 className="text-lg font-medium text-primary dark:text-white mb-3 sm:mb-4">Market Snapshot</h3>
          {loading ? (
            <div className="text-center py-4">
              <p className="text-gray-600 dark:text-gray-400 text-responsive">Loading market data...</p>
            </div>
          ) : error ? (
            <div className="text-center py-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {marketData.map(item => (
                <div key={item.id} className="card p-3 sm:p-4">
                  <h4 className="text-primary dark:text-white font-medium text-sm sm:text-base">{item.name}</h4>
                  <div className="flex justify-between items-center mt-2">
                    <p className="text-lg sm:text-xl font-bold text-primary dark:text-white">{item.value}</p>
                    <span className={`text-xs sm:text-sm ${item.isPositive ? 'text-green-600' : 'text-red-600'}`}>
                      {item.change}
                    </span>
                  </div>
                </div>
        ...
        {/* Financial Tools */}
        <div className="mb-6 sm:mb-8">
          <h3 className="text-lg font-medium text-primary dark:text-white mb-3 sm:mb-4">Financial Tools</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
            {toolsData.map(tool => (
              <Link key={tool.id} to={tool.path} className="card p-4 sm:p-6 hover:shadow-md transition-shadow">
                <div className="text-2xl sm:text-3xl mb-3 sm:mb-4">{tool.icon}</div>
                <h4 className="text-lg sm:text-xl font-medium text-primary dark:text-white mb-1 sm:mb-2">{tool.name}</h4>
        ...
        <div className="sm:hidden mobile-nav fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 z-50 flex justify-around items-center shadow-lg">
          <Link to="/dashboard" className="mobile-nav-item flex flex-col items-center justify-center py-2 flex-1">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span className="text-xs">Home</span>
          </Link>
          <Link to="/chatbot" className="mobile-nav-item flex flex-col items-center justify-center py-2 flex-1">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
            </svg>
            <span className="text-xs">FinBot</span>
          </Link>
          <Link to="/tools/sip-calculator" className="mobile-nav-item flex flex-col items-center justify-center py-2 flex-1">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
            <span className="text-xs">Tools</span>
          </Link>
          <Link to="/profile" className="mobile-nav-item flex flex-col items-center justify-center py-2 flex-1">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
